  ...
  describe("join", () => {
    it("should INCR and SETEX when user is new", async () => {
      ...
      expect(mockRedis.sAdd).toHaveBeenCalledWith(
        "stream:stream-1:viewers",
        "user-1",
      );
      expect(mockRedis.incr).toHaveBeenCalledWith(
        "stream:stream-1:viewer_count",
      );
      expect(mockRedis.setEx).toHaveBeenCalledWith(
        "stream:stream-1:viewer:user-1",
        60,
        "alive",
      );
    });
    ...
    it("should throw when stream is not active", async () => {
      (mockRedis.exists as jest.Mock).mockResolvedValue(0);

      await expect(repo.join("stream-1", "user-1")).rejects.toThrow(
        "Stream not active",
      );
    });
  });
